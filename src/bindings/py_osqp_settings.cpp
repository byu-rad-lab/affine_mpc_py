...
{
  py::class_<OSQPSettings> s(m, "OSQPSettings", R"(
...
Attributes:
  ...
  py::enum_<osqp_linsys_solver_type>(s, "LinsysSolverType")
      .value("OSQP_DIRECT_SOLVER", osqp_linsys_solver_type::OSQP_DIRECT_SOLVER)
      .value("OSQP_INDIRECT_SOLVER",
             osqp_linsys_solver_type::OSQP_INDIRECT_SOLVER)
      .value("OSQP_UNKNOWN_SOLVER",
             osqp_linsys_solver_type::OSQP_UNKNOWN_SOLVER);

  s.def_static(
      "fromOSQPDefaults", &OSQPSolver::getDefaultSettings,
      "Factory method to create OSQPSettings with OSQP default values.");

  s.def(py::init<>([]() { return OSQPSolver::getRecommendedSettings(); }),
